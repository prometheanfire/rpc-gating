def webhooks(){
  stage("Ensure Instances"){
    common.prep_ansible_dynamic_inventory_env()
    common.venvPlaybook(
      playbooks: [
        "rpc-gating/playbooks/setup_openstack_instances.yml"
      ],
      args: [
        "-v",
        "-e '@${env.WORKSPACE}/rpc-gating/instances/webhooks.yml'"
      ]
    ) //venvPlaybook
  }
  stage("Deploy Webhook Translator"){
    withCredentials([
      file(
  ...
  stage("Configure Jenkins Master"){
...
exec ssh root@$ip -i \$keyfile -R 8888:localhost:443 -R 8889:localhost:8888 -N
